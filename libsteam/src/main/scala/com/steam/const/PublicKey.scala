...
/** Public keys for cryptography / encrypted messages
  *
  * {{{
  * val key = PublicKey.getKey(EUniverse.Public)
  * }}}
  */
object PublicKey {
  private val inner = HashMap(
    EUniverse.Public -> Array(
  ...
  def getKey(k: EUniverse.EUniverse): Array[Byte] = inner(k)
}
